//
//  YimaiExaminationTableViewQuestionAnalysisCell
//  DoctorPlatForm
//
...
@interface YimaiExaminationTableViewQuestionAnalysisCell ()<ZLPhotoPickerBrowserViewControllerDataSource,ZLPhotoPickerBrowserViewControllerDelegate>

@property (nonatomic, strong) PhotoContainer *photoC;

@property (nonatomic, strong) NSMutableArray *arrUrl;

@property (weak, nonatomic) IBOutlet UILabel *correct_answerLabel;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *analysisHeight;

@end
@implementation YimaiExaminationTableViewQuestionAnalysisCell

static NSString *const identifier = @"YimaiExaminationTableViewQuestionAnalysisCell";

+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    YimaiExaminationTableViewQuestionAnalysisCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
...
- (void)setFrameModel:(YimaiExaminationQuestionFrameModel *)frameModel
{
    self.correct_answerLabel.text = [NSString stringWithFormat:@"正确答案: %@",frameModel.questionModel.correct_answer];
    
    [self setCoreTextString:[NSString stringWithFormat:@"解析: %@",frameModel.questionModel.question_analysis] textView:self.question_analysisLabelView];
    self.analysisHeight.constant = frameModel.analysisHeight;
}
