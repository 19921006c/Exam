...
- (void)displayImage {
    ...
    if (img) {
        ...
        if (img.size.height/img.size.width>2.0) {
            _photoImageView.image = img;
            _photoImageView.hidden = NO;
            _photoImageView.width = kScreenWidth;
            _photoImageView.height = kScreenWidth / img.size.width * img.size.height;
            CGRect photoImageViewFrame;
            photoImageViewFrame = _photoImageView.frame;
            photoImageViewFrame.origin = CGPointZero;
            _photoImageView.frame = photoImageViewFrame;
            self.contentSize = _photoImageView.size;
        }else{
            _photoImageView.image = img;
            _photoImageView.hidden = NO;
            // Setup photo frame
